...
joinBtn.addEventListener('click', () => {
  ...
  socket.emit('joinRoom', { roomCode, playerName, preferredTeam }, (res) => {
    ...
    if (res.rejoined) {
      lobbyStatus.textContent = `Ponovo si ušao u sobu ${currentRoom} na mjestu igrača ${myIndex}.`;
    } else {
...
// Pokušaj ponovnog ulaska u sobu na osnovu zadnjih sačuvanih podataka
function attemptRejoinFromStorage(showMessage = true) {
  const savedRoom = currentRoom || localStorage.getItem('karte_room');
  const savedName = localStorage.getItem('karte_name');
  const savedTeam = localStorage.getItem('karte_team') || 'A';

  if (!savedRoom || !savedName) {
    if (showMessage) {
      statusBar.textContent = 'Nema sačuvanih podataka za ponovno spajanje.';
    }
    if (currentRoom) {
      socket.emit('requestGameState', { roomCode: currentRoom });
    }
  ...
  socket.emit(
    'joinRoom',
    { roomCode: savedRoom, playerName: savedName, preferredTeam: savedTeam },
    (res) => {
      if (!res || !res.success) {
        if (showMessage) {
          statusBar.textContent =
            (res && res.message) || 'Ponovno spajanje nije uspjelo.';
        }
        return;
      }
      ...
      if (showMessage) {
        statusBar.textContent = res.rejoined
          ? `Ponovo si ušao u sobu ${currentRoom} na mjestu igrača ${myIndex}.`
          : `Spojen u sobu: ${currentRoom}. Tvoj broj igrača: ${myIndex}.`;
      }
...
refreshStateBtn.addEventListener('click', () => {
  // Dugme "Osvježi igru" sada pokušava ponovo spojiti igrača u sobu
  attemptRejoinFromStorage(true);
});
  ...
  socket.emit('listRooms', (rooms) => {
    ...
    roomsList.innerHTML = rooms.map(r => {
      const players = (r.players || [])
        ...
        .map(p => p.name)
        .join(', ') || 'nema igrača';
      const status = r.inProgress ? 'u toku' : 'čeka';
      return `${r.code}: [${players}] – ${status}`;
    }).join('<br>');
  });
}
...
function renderGame(state) {
  ...
  document.body.classList.toggle('game-active', state.phase && state.phase !== 'lobby');
  ...
  const roundA = state.roundCardsA || 0;
  const roundB = state.roundCardsB || 0;

  scoreAEl.textContent =
    `Tim A: ${name0} i ${name2}: ${state.teams.scoreA} (karte u ovoj rundi: ${roundA})`;

  scoreBEl.textContent =
    `Tim B: ${name1} i ${name3}: ${state.teams.scoreB} (karte u ovoj rundi: ${roundB})`;
  ...
  highlightCurrentTurn(state);
}

function createFrontCardDiv(label, suit) {
  const div = document.createElement('div');
  div.classList.add('card');
  div.classList.add(isRedSuit(suit) ? 'red' : 'black');

  const cornerTL = document.createElement('div');
  cornerTL.classList.add('card-corner', 'tl');
  cornerTL.textContent = label;

  const cornerBR = document.createElement('div');
  cornerBR.classList.add('card-corner', 'br');
  cornerBR.textContent = label;
...
function renderHands(state) {
  for (let p = 0; p < 4; p++) {
    ...
    hand.forEach((card) => {
      ...
      if (!isMe) {
        div = createBackCardDiv();
      } else {
        div = createFrontCardDiv(label, card.suit);
...
function renderTablePile(state) {
  ...
  const maxHeight = 140;
  const step = n > 1 ? Math.min(18, maxHeight / (n - 1)) : 0;
  const totalHeight = step * (n - 1);
  const startTop = (maxHeight - totalHeight) / 2;

  pile.forEach((entry, i) => {
    const label = makeLabel(entry.card.rank, entry.card.suit);
    const div = createFrontCardDiv(label, entry.card.suit);
    div.classList.add('pile-card');

    const offset = startTop + i * step;
...
function highlightCurrentTurn(state) {
  if (!state || !state.players) return;
  Object.keys(nameEls).forEach((idx) => {
    const el = nameEls[idx];
    if (!el) return;
    el.classList.toggle('turn', Number(idx) === state.currentTurn);
  });
}
