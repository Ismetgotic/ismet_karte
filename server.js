...
const rooms = {}; // roomCode -> { players: [p0..p3], game }
...
function createRoom() {
  return {
    players: [null, null, null, null], // { id, name, connected }
...
function startGame(room) {
  ...
  // moraju postojati 4 zauzete stolice (bez obzira da li su trenutno online)
  const filledSeats = room.players.filter(p => p !== null).length;
  if (filledSeats !== NUM_PLAYERS) return;

  // ako je prethodna partija završila, resetuj bodove
  ...
  // prvi špil uvijek kreće od seat 0, poslije se deckStarter pomjera u collectTrickAndMaybeScore
  if (typeof game.deckStarter !== 'number') {
    game.deckStarter = 0;
  }
  ...
  if (game.table.length === 1) {
    ...
    if (card.rank === game.firstCardRank || card.rank === '7') {
      if (!sameTeam(game, playerIndex, game.startingPlayer)) {
        game.someoneElseTookLead = true; // protivnička ekipa drži vodstvo
      } else {
        game.someoneElseTookLead = false; // starterov tim opet na čelu
      }
      game.leadOwner = playerIndex;
    }
  }
...
function collectTrickAndMaybeScore(game, winnerIndex) {
  ...
  if (allEmpty) {
    ...
    // BONUS: ako ekipa uzme svih 9 poena, još +10
    if (ptsA === 90) ptsA += 10;
    if (ptsB === 90) ptsB += 10;
    ...
    // servis: ako je ekipa servisera izgubila, servis ide udesno
...
function handlePickupDecision(room, playerIndex, choice) {
  ...
  if (choice === 'me') {
    ...
    // "Nosi" – nosi onaj ko trenutno drži vodstvo
...
io.on('connection', (socket) => {
  ...
  socket.on('joinRoom', ({ roomCode, playerName, preferredTeam }, callback) => {
    ...
    if (existingIndex !== -1) {
      ...
      // ako je igra u toku, pošalji mu trenutno stanje
      if (game.phase !== 'lobby') {
    ...
    // 2) NOVI IGRAČ – dozvoljeno samo dok je igra u lobby fazi
    if (game.phase !== 'lobby') {
    ...
    // ako su sada popunjene sve 4 stolice – startamo igru
    const filledSeats = room.players.filter(p => p !== null).length;
  ...
  socket.on('disconnect', () => {
    ...
    for (const [code, room] of Object.entries(rooms)) {
      ...
      if (idx !== -1) {
        ...
        // sobe za sada ne brišemo automatski
        break;
      }
    }
  });
